...
const AVAILABLE_AGENTS: AgentConfig[] = [
  {
    id: "cuisinier",
    name: "Cuisinier Agent",
    description: "Agent spécialisé en cuisine et gastronomie",
  },
...
async function loadAgentsConfig(): Promise<AgentConfig[]> {
  try {
    console.log(
      `✅ ${AVAILABLE_AGENTS.length} agent(s) disponible(s):`,
      AVAILABLE_AGENTS.map((a) => a.id).join(", ")
    );
...
function createAgent(agentId: string): IAgent {
  ...
  // Exemple:
  // Retourner l'agent correspondant selon l'ID
  // }

  console.log(
    `⚠️  Utilisation du MockAgent pour ${agentId}. Pour utiliser les vrais agents, utilisez server.mts à la place.`
  );
    ...
    try {
      ...
      if (!agentConfig) {
        return res.status(404).json({
          ...
          message: `L'agent '${agentId}' n'existe pas. Agents disponibles: ${agents
            .map((a) => a.id)
            .join(", ")}`,
        });
      }
